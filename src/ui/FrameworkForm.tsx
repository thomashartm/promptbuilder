...
export default function FrameworkForm() {
  ...
  const generatePrompt = () => {
    const pieces = framework.parameters.map(p => `${p.name}: ${inputs[p.name] || ''}`);
    setResult(`${framework.id}:\n` + pieces.join('\n'));
  };
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex space-x-2">
        <button className="border px-2" onClick={generatePrompt}>Generate</button>
        <button className="border px-2" onClick={copyPrompt}>Copy</button>
        <button className="border px-2" onClick={save}>Save</button>
      </div>

      <textarea className="border p-1 w-full" rows={6} value={result} readOnly />
    </div>
  );
}
